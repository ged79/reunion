...
// 회원 명부 공용 컴포넌트 — 청년회원(/members)과 협의회원(/council) 페이지에서 공유
// memberType으로 조회 대상을 구분하고, 제목/부제목만 페이지별로 다르게 표시
...
import { fetchMembers, getSessionUser, type Member, type MemberType } from '@/lib/supabase'
...
export default function MemberDirectory({
  ...
  useEffect(() => {
    if (authState === 'member') fetchMembers(memberType).then(setMembers)
  }, [authState, memberType])
    ...
    <div>
      ...
      <div className="bg-gradient-to-b from-gray-50 to-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 py-12">
          ...
          <p className="text-gray-500 ml-4">{subtitle}</p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        {/* 직업군 필터 드롭다운 — 인원이 적은 명부(협의회원)에서는 숨김 */}
        {showCategoryFilter && (
        <div className="mb-6">
          <select
            value={active}
