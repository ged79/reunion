import { getBranch } from '@/lib/mockData'
import BranchNav from '@/components/BranchNav'
import { notFound } from 'next/navigation'
import Link from 'next/link'
import type { Metadata } from 'next'

export async function generateMetadata({ params }: { params: { branch: string } }): Promise<Metadata> {
  const branch = getBranch(params.branch)
  if (!branch) return {}
  return {
    title: branch.name,
    description: branch.description || `${branch.name} 공식 홈페이지`,
    openGraph: {
      title: branch.name,
      description: branch.description || `${branch.name} 공식 홈페이지`,
      locale: 'ko_KR',
    },
  }
}

export default async function BranchLayout({
  children,
  params,
}: {
  children: React.ReactNode
  params: { branch: string }
}) {
  const branch = getBranch(params.branch)
  if (!branch) notFound()

  return (
    <div className="min-h-screen flex flex-col">
      <BranchNav branch={branch} />
      <main className="flex-1">{children}</main>
      <footer className="bg-gray-900 text-gray-300 mt-16">
        <div className="max-w-6xl mx-auto px-4 py-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              <p className="font-black text-base mb-1">
                <span style={{ color: branch.color }}>민통</span>
                <span className="text-white ml-1">{branch.name.replace('민족통일청년회', '').trim()}</span>
              </p>
              <p className="text-sm text-gray-400">{branch.region}</p>
            </div>
            <div>
              <h4 className="font-semibold text-white text-sm mb-3">바로가기</h4>
              <ul className="space-y-1 text-sm">
                <li><Link href={`/${branch.slug}/members`} className="hover:text-white transition-colors">회원</Link></li>
                <li><Link href={`/${branch.slug}/photos`} className="hover:text-white transition-colors">갤러리</Link></li>
                <li><Link href={`/${branch.slug}/updates`} className="hover:text-white transition-colors">소식</Link></li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold text-white text-sm mb-3">연락처</h4>
              {branch.contact_email && (
                <p className="text-sm mb-1">
                  이메일:{' '}
                  <a href={`mailto:${branch.contact_email}`} className="hover:text-white transition-colors">
                    {branch.contact_email}
                  </a>
                </p>
              )}
              {branch.contact_phone && (
                <p className="text-sm">전화: {branch.contact_phone}</p>
              )}
            </div>
          </div>
          {/* SNS Links */}
          {(branch.youtube_url || branch.facebook_url || branch.instagram_url) && (
            <div className="border-t border-gray-700 mt-8 pt-6 flex justify-center gap-3">
              {branch.youtube_url && (
                <a
                  href={branch.youtube_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 rounded-full bg-gray-700 hover:bg-red-600 flex items-center justify-center transition-colors"
                  aria-label="YouTube"
                >
                  <svg className="w-4 h-4 text-white" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                  </svg>
                </a>
              )}
              {branch.facebook_url && (
                <a
                  href={branch.facebook_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 rounded-full bg-gray-700 hover:bg-blue-600 flex items-center justify-center transition-colors"
                  aria-label="Facebook"
                >
                  <svg className="w-4 h-4 text-white" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                </a>
              )}
              {branch.instagram_url && (
                <a
                  href={branch.instagram_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 rounded-full bg-gray-700 hover:bg-pink-600 flex items-center justify-center transition-colors"
                  aria-label="Instagram"
                >
                  <svg className="w-4 h-4 text-white" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                  </svg>
                </a>
              )}
            </div>
          )}
          <div className="border-t border-gray-700 mt-6 pt-6 text-center text-xs text-gray-500">
            &copy; 2025 {branch.name}. All rights reserved.
          </div>
        </div>
      </footer>
    </div>
  )
}
