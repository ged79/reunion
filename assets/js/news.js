...
async function createNews(newsData) {
  try {
    ...
    if (newsData.category === '행사안내') {
      insertData.event_date = newsData.event_date || null;
      insertData.location = newsData.location || null;
    }
...
async function submitEdit() {
  try {
    ...
    // 행사안내 카테고리인 경우 행사 필드 추가
    if (category === '행사안내') {
      var eventDateVal = document.getElementById('edit_event_date').value;
      updateData.event_date = eventDateVal || null;
      updateData.location = document.getElementById('edit_location').value || null;
    } else {
